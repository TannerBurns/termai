...
final class ChatSession: ObservableObject, Identifiable, ShellCommandExecutor, PlanTrackDelegate, CreatePlanDelegate {
    ...
    private func executeStepWithNativeTools(
        ...
        // Also add attached contexts from the MOST RECENT user message (for files, terminal output, etc.)
        if let lastUserMessage = messages.filter({ $0.role == "user" }).last {
            // Add terminal context if present (this is the primary terminal output attached to the message)
            ...
            // Add other attached contexts (files, etc.)
